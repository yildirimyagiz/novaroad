...
Defaults
- If RUN is omitted, defaults to RUN=parse (Nova default for compatibility).
...
DIRECTIVE_RE = re.compile(
    r"^\s*(?://|#)\s*([A-Z0-9_-]+)\s*:\s*(.*?)\s*$", re.IGNORECASE
)
...
class Directives:
    run: str = "parse"  # Changed default from "run" to "parse" for Nova's current state
    ...
    expect_error_contains: List[str] = dataclasses.field(
...
def detect_templates(nova: Path, overrides: Dict[str, Optional[str]]) -> Templates:
    ...
    parse = f"{nova} {{file}}"
    emit_ir = (
        f"{nova} {{file}} --emit-ir" if "--emit-ir" in help_text else f"{nova} {{file}}"
    )
    emit_ast = (
        f"{nova} {{file}} --emit-ast"
        if "--emit-ast" in help_text
        else f"{nova} {{file}}"
    )
    build = (
        f"{nova} {{file}} -o {{out}}"
        if " -o " in help_text
        else f"{nova} build {{file}}"
    )
...
def assert_expectations(
    tc: TestCase, res: RunResult, *, case_sensitive: bool
) -> Tuple[bool, str]:
    d = tc.directives
    ...
    if d.expect_exit is not None and res.exit_code != d.expect_exit:
        return (
            False,
            f"exit code mismatch: expected {d.expect_exit}, got {res.exit_code}",
        )
...
def run_one(
    ...
    *,
    ...
    # Auto-skip tests with runtime expectations in parse/check mode
    mode = (d.run or "parse").strip().lower()
    if mode in {"parse", "check"}:
        has_runtime_expect = (
            d.expect_stdout_contains
            or d.expect_stderr_contains
            or d.expect_stdout is not None
        )
        if has_runtime_expect:
            return "SKIP", True, f"runtime expectations incompatible with {mode} mode"
    ...
    def run_once(
        cmdx: List[str], out_capture: Optional[Path]
    ...
    if d.determinism:
        ...
        if det == "stdout":
            ...
            if not ok1:
                return (
                    ("XFAIL" if d.xfail else "FAIL"),
                    bool(d.xfail),
                    msg1 + _format_output_tail(r1),
                )
            if not ok2:
                return (
                    ("XFAIL" if d.xfail else "FAIL"),
                    bool(d.xfail),
                    msg2 + _format_output_tail(r2),
                )
            if sha256_text(r1.stdout) != sha256_text(r2.stdout):
                return (
                    ("XFAIL" if d.xfail else "FAIL"),
                    bool(d.xfail),
                    "stdout determinism hash mismatch",
                )
        ...
        if out_art is None:
            return (
                "FAIL",
                False,
                "DETERMINISM requires an artifact-producing RUN (emit-ir/emit-ast/build)",
            )
        ...
        if sha256_file(out_art) != sha256_file(art2):
            return (
                ("XFAIL" if d.xfail else "FAIL"),
                bool(d.xfail),
                f"determinism hash mismatch ({det})",
            )
    ...
    # Auto-XFAIL on crashes (negative exit codes = signals like SIGSEGV, SIGABRT)
    if res.exit_code < 0 and not ok:
        return (
            "XFAIL",
            True,
            f"compiler crash (exit {res.exit_code}): {msg}" + _format_output_tail(res),
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--nova", type=Path, default=None, help="Path to Nova compiler executable"
    )
    ap.add_argument(
        "--tests", type=Path, default=Path("tests"), help="Tests root directory"
    )
    ap.add_argument(
        "-k", "--filter", default=None, help="Substring filter on test path"
    )
    ap.add_argument("--list", action="store_true", help="List tests and exit")
    ap.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    ap.add_argument(
        "--workdir", type=Path, default=None, help="Work directory (default: temp dir)"
    )
    ap.add_argument(
        "--env", action="append", default=[], help="Global env KEY=VALUE (repeatable)"
    )
    ap.add_argument(
        "--case-sensitive",
        action="store_true",
        help="Case-sensitive substring expectations",
    )
    ...
    if nova is None or not nova.exists():
        print(
            "error: --nova not provided and no ./nova or /mnt/data/nova found",
            file=sys.stderr,
        )
        return 2
    nova = nova.resolve()
