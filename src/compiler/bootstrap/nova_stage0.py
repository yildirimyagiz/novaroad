...
class CodeGen:
    def __init__(self):
        ...
        self.var_types: Dict[str, str] = {}  # var name -> zn type name
...
"""
        ...
        if isinstance(node, LetStmt):
            if isinstance(node.name, list):
                ...
                if node.value:
                    ...
                    zn_type = node.type_ann
                    if not zn_type:
                        zn_type = self.infer_zn_type(node.value)
                    
                    ctype = self.type_to_c(zn_type) if zn_type else "int64_t"
                    self.var_types[node.name] = zn_type if zn_type else "int"
                    self.emit(f"{ctype} {self.safe_name(node.name)} = {val};")
                else:
                    self.var_types[node.name] = node.type_ann if node.type_ann else "int"
    ...
    def infer_zn_type(self, node: ASTNode) -> Optional[str]:
        ...
        if isinstance(node, FormatStr): return "str"
        if isinstance(node, StructLit): return node.name
        if isinstance(node, Call):
            if isinstance(node.func, FieldAccess):
                # Type::new()
                return self._expr_to_path_str(node.func.obj).split("::")[0]
    ...
    def infer_c_type(self, node: ASTNode) -> str:
        zn = self.infer_zn_type(node)
        if zn: return self.type_to_c(zn)
        return "int64_t"
    ...
    def gen_method_call(self, node: MethodCall) -> str:
        ...
        if isinstance(node.obj, Identifier):
            struct_name = self.var_types.get(node.obj.name, "void")
        
        if struct_name == "void":
             # Fallback to object name if type unknown
             all_args = ", ".join([obj] + ([args_str] if args_str else []))
             return f"{obj}__{node.method}({all_args})"
        
        all_args = ", ".join([obj] + ([args_str] if args_str else []))
        return f"{struct_name}__{node.method}({all_args})"
...
def compile_files(
    ...
    print(f"   Modules: {len(source_paths)} files")
    ...
    # Only link net runtime if it exists
...
def main():
    ...
    success = compile_files(
        all_files, output, verbose=args.verbose, keep_c=args.keep_c, only_c=args.emit_c
    )
