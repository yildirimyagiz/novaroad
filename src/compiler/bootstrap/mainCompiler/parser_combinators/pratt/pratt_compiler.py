...
CASES: List[Tuple[str, List[str], str]] = [
    ("Associativity", ['Left', 'Right'], "Associativity definition"),
    ("ArithExpr", ['Num(i64)', 'Var(String)', 'Add(Box<ArithExpr>, Box<ArithExpr>)', 'Sub(Box<ArithExpr>, Box<ArithExpr>)', 'Mul(Box<ArithExpr>, Box<ArithExpr>)', 'Div(Box<ArithExpr>, Box<ArithExpr>)', 'Neg(Box<ArithExpr>)'], "ArithExpr definition"),
]

@dataclass
class PrattCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_PRATT_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = PrattCompilerGenerator().compile()
