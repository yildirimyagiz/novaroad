...
CASES: List[Tuple[str, List[str], str]] = [
    ("ParseResult", ['Success(T, &[Token])', 'Failure(ParseError)'], "ParseResult definition"),
    ("TokenKind", ['Fn', 'Let', 'If', 'Else', 'While', 'For', 'Match', 'Return', 'Plus', 'Minus', 'Star', 'Slash', 'Eq', 'EqEq', 'Lt', 'Gt', 'LParen', 'RParen', 'LBrace', 'RBrace', 'LBracket', 'RBracket', 'Comma', 'Semicolon', 'Colon', 'Arrow', 'IntLit(i64)', 'StringLit(String)', 'BoolLit(bool)', 'Ident(String)', 'Eof'], "TokenKind definition"),
]

@dataclass
class CoreCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CORE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = CoreCompilerGenerator().compile()
