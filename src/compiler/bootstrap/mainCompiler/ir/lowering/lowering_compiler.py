...
CASES: List[Tuple[str, List[str], str]] = [
    ("ASTNode", ['VarDecl { name: String, ty: ASTType, init: Option<Box<ASTNode>> }', 'Assignment { target: String, value: Box<ASTNode> }', 'Return { value: Option<Box<ASTNode>> }', 'If { cond: Box<ASTNode>, then_branch: Box<ASTNode>, else_branch: Option<Box<ASTNode>> }', 'While { cond: Box<ASTNode>, body: Box<ASTNode> }', 'Block { stmts: Vec<ASTNode> }', 'IntLiteral(i64)', 'FloatLiteral(f64)', 'BoolLiteral(bool)', 'Variable(String)', 'BinaryOp { op: String, lhs: Box<ASTNode>, rhs: Box<ASTNode> }', 'Call { func: String, args: Vec<ASTNode> }'], "ASTNode definition"),
    ("ASTType", ['Int', 'Float', 'Bool', 'Void'], "ASTType definition"),
]

@dataclass
class LoweringCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_LOWERING_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = LoweringCompilerGenerator().compile()
