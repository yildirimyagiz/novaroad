...
CASES: List[Tuple[str, List[str], str]] = [
    ("Result", ['Ok(T)', 'Err(E)'], "Result definition"),
]

@dataclass
class ResultCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_RESULT_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = ResultCompilerGenerator().compile()
