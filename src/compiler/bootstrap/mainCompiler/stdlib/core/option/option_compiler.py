...
CASES: List[Tuple[str, List[str], str]] = [
    ("Option", ['Some(T)', 'None'], "Option definition"),
]

@dataclass
class OptionCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_OPTION_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = OptionCompilerGenerator().compile()
