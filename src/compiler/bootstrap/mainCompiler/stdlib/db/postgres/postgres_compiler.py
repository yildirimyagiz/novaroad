...
CASES: List[Tuple[str, List[str], str]] = [
    ("PgValue", ['Null', 'Bool(bool)', 'Int(i64)', 'Float(f64)', 'Text(String)', 'Bytes(Vec<u8>)'], "PgValue definition"),
]

@dataclass
class PostgresCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_POSTGRES_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = PostgresCompilerGenerator().compile()
