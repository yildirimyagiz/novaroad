...
CASES: List[Tuple[str, List[str], str]] = [
    ("Method", ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS'], "Method definition"),
    ("StatusCode", ['OK = 200', 'Created = 201', 'NoContent = 204', 'MovedPermanently = 301', 'Found = 302', 'BadRequest = 400', 'Unauthorized = 401', 'Forbidden = 403', 'NotFound = 404', 'InternalServerError = 500'], "StatusCode definition"),
]

@dataclass
class HttpCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_HTTP_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = HttpCompilerGenerator().compile()
