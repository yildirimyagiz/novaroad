...
CASES: List[Tuple[str, List[str], str]] = [
    ("TargetPlatform", ['CPU', 'GPU(GPUVendor)', 'TPU', 'NPU', 'Unified'], "TargetPlatform definition"),
    ("GPUVendor", ['NVIDIA', 'AMD', 'Intel', 'Apple'], "GPUVendor definition"),
    ("MultilingualError", ['UnsupportedLanguage(String)', 'TranslationError(String)', 'CulturalAdaptationError(String)', 'CompilationError(String)', 'FallbackError(String)'], "MultilingualError definition"),
]

@dataclass
class MultilingualCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_MULTILINGUAL_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_MULTILINGUAL_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = MultilingualCompilerGenerator().compile()
