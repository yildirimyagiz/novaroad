...
CASES: List[Tuple[str, List[str], str]] = [
    ("LifetimeStyle", ['Inferred', 'Explicit', 'Elided'], "LifetimeStyle definition"),
    ("MobileLifetimeViolation", ['BatteryConstrainedLifetime(String, f64)', 'ThermalConstrainedLifetime(String, String)', 'PermissionDeniedLifetime(String, String)', 'PlatformIncompatibleLifetime(String, String)', 'CapabilityMissingLifetime(String, String)', 'LifetimeTooLong(String, i64)'], "MobileLifetimeViolation definition"),
]

@dataclass
class LifetimeAnnotationsCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_LIFETIME_ANNOTATIONS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_LIFETIME_ANNOTATIONS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = LifetimeAnnotationsCompilerGenerator().compile()
