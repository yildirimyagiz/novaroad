...
CASES: List[Tuple[str, List[str], str]] = [
    ("MobileEscapeViolation", ['BatteryConstrainedEscape(String, f64)', 'ThermalConstrainedEscape(String, String)', 'PermissionDeniedEscape(String, String)', 'PlatformIncompatibleEscape(String, String)', 'CapabilityMissingEscape(String, String)'], "MobileEscapeViolation definition"),
]

@dataclass
class EscapeAnalysisCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_ESCAPE_ANALYSIS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_ESCAPE_ANALYSIS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = EscapeAnalysisCompilerGenerator().compile()
