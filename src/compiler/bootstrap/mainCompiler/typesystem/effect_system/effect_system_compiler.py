...
CASES: List[Tuple[str, List[str], str]] = [
    ("MobileEffectViolation", ['PermissionDenied(String)', 'InsufficientBattery(f64, f64)', 'ThermalConstraintViolated(String)', 'PlatformMismatch(String, String)', 'CapabilityUnavailable(String)', 'ResourceLimitExceeded(String)', 'DeviceStateIncompatible(String)'], "MobileEffectViolation definition"),
]

@dataclass
class EffectSystemCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_EFFECT_SYSTEM_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_EFFECT_SYSTEM_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = EffectSystemCompilerGenerator().compile()
