...
CASES: List[Tuple[str, List[str], str]] = [
    ("EqualityProof", ['Refl(Term)', 'Sym(Box<EqualityProof>)', 'Trans(Box<EqualityProof>, Box<EqualityProof>)', 'Cong(Box<Term>, Box<EqualityProof>)', 'Beta(Term, Term, Term)', 'Delta(String, Term)', 'MobilePermission(String, Box<EqualityProof>)', 'MobileCapability(String, Box<EqualityProof>)', 'BatteryAware(f64, Box<EqualityProof>, Box<EqualityProof>)', 'ThermalAware(String, Box<EqualityProof>, Box<EqualityProof>)'], "EqualityProof definition"),
]

@dataclass
class EqualityTypeCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_EQUALITY_TYPE_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_EQUALITY_TYPE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = EqualityTypeCompilerGenerator().compile()
