...
CASES: List[Tuple[str, List[str], str]] = [
    ("ConstraintKind", ['Outlives(Origin, Origin)', 'PathHasOrigin(Path, Origin)', 'CallPropagate(Origin, Origin)'], "ConstraintKind definition"),
]

@dataclass
class OriginInferenceCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_ORIGIN_INFERENCE_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_ORIGIN_INFERENCE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = OriginInferenceCompilerGenerator().compile()
