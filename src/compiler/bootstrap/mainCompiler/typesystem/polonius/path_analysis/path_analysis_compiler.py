...
CASES: List[Tuple[str, List[str], str]] = [
    ("ConditionKind", ['IsTrue(LocalId)', 'IsFalse(LocalId)', 'Equals(LocalId, LocalId)', 'NotEquals(LocalId, LocalId)', 'IsSome(LocalId)', 'IsNone(LocalId)'], "ConditionKind definition"),
    ("PredicateKind", ['NonNull', 'Initialized', 'Moved', 'Borrowed'], "PredicateKind definition"),
]

@dataclass
class PathAnalysisCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_PATH_ANALYSIS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_PATH_ANALYSIS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = PathAnalysisCompilerGenerator().compile()
