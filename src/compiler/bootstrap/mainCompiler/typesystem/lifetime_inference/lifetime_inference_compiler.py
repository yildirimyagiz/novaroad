...
CASES: List[Tuple[str, List[str], str]] = [
    ("LifetimeRelation", ['Outlives', 'Equals', 'Contains'], "LifetimeRelation definition"),
    ("MobileLifetimeViolation", ['BatteryConstrainedInference(String, f64)', 'ThermalConstrainedInference(String, String)', 'PermissionDeniedInference(String, String)', 'PlatformIncompatibleInference(String, String)', 'CapabilityMissingInference(String, String)'], "MobileLifetimeViolation definition"),
]

@dataclass
class LifetimeInferenceCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_LIFETIME_INFERENCE_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_LIFETIME_INFERENCE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = LifetimeInferenceCompilerGenerator().compile()
