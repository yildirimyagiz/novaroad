...
CASES: List[Tuple[str, List[str], str]] = [
    ("Effect", ['Pure', 'IO', 'Diverge', 'Panic', 'Async', 'Custom(String)', 'Var(EffectVar)', 'Union(Vec<Effect>)'], "Effect definition"),
    ("EffectConstraint", ['Eq(Effect, Effect)', 'Sub(Effect, Effect)', 'Bound(EffectVar, Vec<Effect>)'], "EffectConstraint definition"),
]

@dataclass
class ConstraintCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_CONSTRAINT_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CONSTRAINT_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = ConstraintCompilerGenerator().compile()
