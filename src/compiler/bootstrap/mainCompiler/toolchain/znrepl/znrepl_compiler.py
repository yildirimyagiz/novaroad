...
CASES: List[Tuple[str, List[str], str]] = [
    ("Value", ['Int(i64)', 'Float(f64)', 'Bool(bool)', 'String(String)', 'Unit'], "Value definition"),
]

@dataclass
class ZnreplCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_ZNREPL_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = ZnreplCompilerGenerator().compile()
