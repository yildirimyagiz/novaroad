...
CASES: List[Tuple[str, List[str], str]] = [
    ("OptimizationLevel", ['O0', 'O1', 'O2', 'O3', 'O4', 'Pgo', 'Lto', 'MobilePgo', 'MobileLto', 'UltimateMobile'], "OptimizationLevel definition"),
    ("MobileOptimizationLevel", ['Standard', 'Performance', 'Battery', 'Balanced'], "MobileOptimizationLevel definition"),
]

@dataclass
class PgoLtoCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_PGO_LTO_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = PgoLtoCompilerGenerator().compile()
