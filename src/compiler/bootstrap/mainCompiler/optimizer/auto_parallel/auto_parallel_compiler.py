...
CASES: List[Tuple[str, List[str], str]] = [
    ("ParallelismKind", ['SIMD', 'DataParallel', 'TaskParallel', 'Pipeline', 'NestedData', 'Recursive', 'UIParallel', 'AnimationParallel', 'MLBatchParallel', 'ARParallel', 'CameraPipeline', 'GestureParallel', 'SensorFusion', 'AccessibilityParallel', 'NetworkBatch', 'BackgroundTaskParallel'], "ParallelismKind definition"),
    ("ParallelizationOpportunity", ['None', 'DataParallel', 'SIMD', 'Reduction', 'TaskParallel', 'UIParallel', 'AnimationParallel', 'MLBatchParallel', 'ARParallel', 'CameraPipeline', 'GestureParallel', 'SensorFusion', 'AccessibilityParallel', 'NetworkBatch', 'BackgroundTaskParallel'], "ParallelizationOpportunity definition"),
]

@dataclass
class AutoParallelCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_AUTO_PARALLEL_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_AUTO_PARALLEL_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = AutoParallelCompilerGenerator().compile()
