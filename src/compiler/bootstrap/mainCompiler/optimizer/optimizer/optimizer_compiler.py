...
CASES: List[Tuple[str, List[str], str]] = [
    ("OptimizeLevel", ['None', 'Standard', 'Supreme', 'Mobile', 'Ultimate'], "OptimizeLevel definition"),
    ("MobileOptimizationTarget", ['Performance', 'BatteryLife', 'FrameRate', 'NetworkEfficiency', 'StartupTime', 'ThermalManagement'], "MobileOptimizationTarget definition"),
]

@dataclass
class OptimizerCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_OPTIMIZER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = OptimizerCompilerGenerator().compile()
