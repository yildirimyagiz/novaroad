...
CASES: List[Tuple[str, List[str], str]] = [
    ("MobileOptimizationTarget", ['Performance', 'BatteryLife', 'MemoryUsage', 'FrameRate', 'NetworkEfficiency', 'StartupTime'], "MobileOptimizationTarget definition"),
]

@dataclass
class OptimizedCompilerCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_OPTIMIZED_COMPILER_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_OPTIMIZED_COMPILER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = OptimizedCompilerCompilerGenerator().compile()
