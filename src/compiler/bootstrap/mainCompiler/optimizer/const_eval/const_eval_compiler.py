...
CASES: List[Tuple[str, List[str], str]] = [
    ("ConstValue", ['Int(i64)', 'Float(f64)', 'Bool(bool)', 'Str(String)', 'Array(Vec<ConstValue>)', 'Struct(HashMap<String, ConstValue>)', 'Point { x: f64, y: f64 }', 'TouchEvent { event_type: String, touches: Vec<ConstValue> }', 'GestureEvent { gesture_type: String, properties: HashMap<String, ConstValue> }', 'SensorData { sensor_type: String, values: Vec<f64> }', 'UIComponent { component_type: String, properties: HashMap<String, ConstValue> }', 'Color { r: f64, g: f64, b: f64, a: f64 }', 'Size { width: f64, height: f64 }', 'Null'], "ConstValue definition"),
]

@dataclass
class ConstEvalCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_CONST_EVAL_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CONST_EVAL_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = ConstEvalCompilerGenerator().compile()
