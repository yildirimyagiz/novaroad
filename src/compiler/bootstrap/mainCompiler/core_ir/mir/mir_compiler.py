...
CASES: List[Tuple[str, List[str], str]] = [
    ("MIRStmt", ['Assign(Place, MIRExpr)', 'Call(Place, String, Vec<Operand>)', 'Nop'], "MIRStmt definition"),
    ("MIRTerminator", ['Return', 'Goto(i64)', 'Branch {\n        cond: Operand,\n        true_bb: i64,\n        false_bb: i64,\n    }', 'Unreachable'], "MIRTerminator definition"),
    ("Operand", ['Copy(Place)', 'Move(Place)', 'Const(IRLit)'], "Operand definition"),
    ("MIRExpr", ['Use(Operand)', 'BinOp(IROp, Operand, Operand)', 'UnOp(IROp, Operand)'], "MIRExpr definition"),
]

@dataclass
class MirCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_MIR_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = MirCompilerGenerator().compile()
