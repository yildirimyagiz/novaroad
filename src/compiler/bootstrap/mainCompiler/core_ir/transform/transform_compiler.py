...
CASES: List[Tuple[str, List[str], str]] = [
    ("MIRPass", ['ConstProp', 'DeadCodeElim', 'HarmonicUnroll', 'SIMDVectorize', 'Inlining'], "MIRPass definition"),
]

@dataclass
class TransformCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_TRANSFORM_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = TransformCompilerGenerator().compile()
