...
CASES: List[Tuple[str, List[str], str]] = [
    ("Target", ['X86_64_Linux', 'X86_64_Windows', 'X86_64_MacOS', 'ARM64_Linux', 'ARM64_MacOS', 'WASM32', 'RISCV64'], "Target definition"),
]

@dataclass
class CrossCompilerCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_CROSS_COMPILER_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CROSS_COMPILER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = CrossCompilerCompilerGenerator().compile()
