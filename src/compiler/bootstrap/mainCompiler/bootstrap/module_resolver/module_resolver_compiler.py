...
CASES: List[Tuple[str, List[str], str]] = [
    ("ModuleError", ['ModuleNotFound(String)', 'ItemNotExported { module: String, item: String }', 'CircularDependency(String)', 'DuplicateModule(String)', 'InvalidModulePath(String)'], "ModuleError definition"),
]

@dataclass
class ModuleResolverCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_MODULE_RESOLVER_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_MODULE_RESOLVER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = ModuleResolverCompilerGenerator().compile()
