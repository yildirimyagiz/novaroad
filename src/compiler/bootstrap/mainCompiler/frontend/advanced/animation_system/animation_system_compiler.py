...
CASES: List[Tuple[str, List[str], str]] = [
    ("AnimationCurve", ['Linear', 'EaseIn', 'EaseOut', 'EaseInOut', 'CubicBezier(f64, f64, f64, f64)', 'Spring { mass: f64, stiffness: f64, damping: f64, velocity: f64 }', 'Bounce', 'Elastic', 'Custom(Box<dyn Fn(f64) -> f64>)'], "AnimationCurve definition"),
    ("RepeatMode", ['Restart', 'Reverse', 'Mirror'], "RepeatMode definition"),
    ("AnimationDirection", ['Normal', 'Reverse', 'Alternate'], "AnimationDirection definition"),
    ("SlideDirection", ['Top', 'Bottom', 'Left', 'Right'], "SlideDirection definition"),
    ("AnimationTrigger", ['OnAppear', 'OnDisappear', 'OnTap', 'OnStateChange', 'Manual'], "AnimationTrigger definition"),
]

@dataclass
class AnimationSystemCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_ANIMATION_SYSTEM_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_ANIMATION_SYSTEM_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = AnimationSystemCompilerGenerator().compile()
