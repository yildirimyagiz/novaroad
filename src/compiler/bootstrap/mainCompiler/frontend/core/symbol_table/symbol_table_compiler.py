...
CASES: List[Tuple[str, List[str], str]] = [
    ("Visibility", ['Private', 'Public', 'Open'], "Visibility definition"),
    ("ScopeKind", ['Global', 'Module', 'Function', 'Block', 'Loop', 'MatchArm'], "ScopeKind definition"),
]

@dataclass
class SymbolTableCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_SYMBOL_TABLE_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_SYMBOL_TABLE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = SymbolTableCompilerGenerator().compile()
