...
CASES: List[Tuple[str, List[str], str]] = [
    ("BackendError", ['UnsupportedFeature(String)', 'TypeMismatch(String)', 'UndefinedSymbol(String)', 'InternalError(String)'], "BackendError definition"),
]

@dataclass
class LlvmBackendCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_LLVM_BACKEND_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_LLVM_BACKEND_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = LlvmBackendCompilerGenerator().compile()
