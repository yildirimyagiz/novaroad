...
CASES: List[Tuple[str, List[str], str]] = [
    ("TextAlignment", ['Left', 'Center', 'Right'], "TextAlignment definition"),
    ("LayoutType", ['Absolute', 'Vertical', 'Horizontal', 'Grid'], "LayoutType definition"),
    ("FontWeight", ['Thin', 'Light', 'Regular', 'Medium', 'Bold', 'Black'], "FontWeight definition"),
    ("FontStyle", ['Normal', 'Italic', 'Oblique'], "FontStyle definition"),
    ("WidgetError", ['CreationFailed', 'InvalidConfig', 'PlatformError'], "WidgetError definition"),
]

@dataclass
class NativeWidgetsCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_NATIVE_WIDGETS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_NATIVE_WIDGETS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = NativeWidgetsCompilerGenerator().compile()
