...
@dataclass
class PlatformBindingsCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_PLATFORM_BINDINGS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_PLATFORM_BINDINGS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = PlatformBindingsCompilerGenerator().compile()
