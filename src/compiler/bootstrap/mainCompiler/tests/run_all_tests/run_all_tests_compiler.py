...
@dataclass
class RunAllTestsCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_RUN_ALL_TESTS_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_RUN_ALL_TESTS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = RunAllTestsCompilerGenerator().compile()
