...
CASES: List[Tuple[str, List[str], str]] = [
    ("SIMDISASet", ['SSE4_2', 'AVX', 'AVX2', 'AVX512', 'NEON', 'SVE'], "SIMDISASet definition"),
]

@dataclass
class SimdCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_SIMD_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = SimdCompilerGenerator().compile()
