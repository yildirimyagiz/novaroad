...
@dataclass
class JitCompilerCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_JIT_COMPILER_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_JIT_COMPILER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = JitCompilerCompilerGenerator().compile()
