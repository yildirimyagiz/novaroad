...
CASES: List[Tuple[str, List[str], str]] = [
    ("Arch", ['X86_64', 'AArch64', 'ARM', 'RISCV64', 'WASM32'], "Arch definition"),
    ("OS", ['Linux', 'Windows', 'MacOS', 'iOS', 'Android', 'FreeBSD', 'None'], "OS definition"),
    ("Environment", ['GNU', 'MSVC', 'Musl', 'Android', 'Darwin', 'None'], "Environment definition"),
]

@dataclass
class CrossCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CROSS_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = CrossCompilerGenerator().compile()
