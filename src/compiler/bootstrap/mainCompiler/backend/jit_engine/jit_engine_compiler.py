...
CASES: List[Tuple[str, List[str], str]] = [
    ("OptLevel", ['O0', 'O1', 'O2', 'O3'], "OptLevel definition"),
]

@dataclass
class JitEngineCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_JIT_ENGINE_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_JIT_ENGINE_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = JitEngineCompilerGenerator().compile()
