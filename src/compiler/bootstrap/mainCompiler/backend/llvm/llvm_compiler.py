...
CASES: List[Tuple[str, List[str], str]] = [
    ("OptLevel", ['O0', 'O1', 'O2', 'O3', 'Os', 'Oz'], "OptLevel definition"),
]

@dataclass
class LlvmCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_LLVM_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = LlvmCompilerGenerator().compile()
