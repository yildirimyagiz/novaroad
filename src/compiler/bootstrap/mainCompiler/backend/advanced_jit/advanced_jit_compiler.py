...
CASES: List[Tuple[str, List[str], str]] = [
    ("JITMode", ['Interpreted', 'LazyJIT', 'AdaptiveJIT', 'AOT'], "JITMode definition"),
    ("JITValue", ['Integer(i64)', 'Float(f64)', 'Boolean(bool)', 'String(String)'], "JITValue definition"),
]

@dataclass
class AdvancedJitCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_ADVANCED_JIT_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_ADVANCED_JIT_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = AdvancedJitCompilerGenerator().compile()
