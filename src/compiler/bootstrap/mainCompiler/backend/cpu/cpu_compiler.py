...
CASES: List[Tuple[str, List[str], str]] = [
    ("CPUArch", ['X86_64', 'ARM64', 'RISCV64'], "CPUArch definition"),
]

@dataclass
class CpuCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_CPU_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = CpuCompilerGenerator().compile()
