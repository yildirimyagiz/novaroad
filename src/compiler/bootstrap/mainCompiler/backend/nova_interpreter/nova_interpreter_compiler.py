...
CASES: List[Tuple[str, List[str], str]] = [
    ("Value", ['Integer(i64)', 'Float(f64)', 'Boolean(bool)', 'String(String)', 'Function { name: String, params: Vec<String>, body: Block }', 'Null'], "Value definition"),
]

@dataclass
class NovaInterpreterCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_NOVA_INTERPRETER_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_NOVA_INTERPRETER_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = NovaInterpreterCompilerGenerator().compile()
