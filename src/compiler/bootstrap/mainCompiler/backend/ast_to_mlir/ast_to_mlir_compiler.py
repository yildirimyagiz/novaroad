...
CASES: List[Tuple[str, List[str], str]] = [
    ("MLIRType", ['I64', 'F64', 'Bool', 'Linear(Box<MLIRType>)', 'Borrowed { target: Box<MLIRType>, lifetime: String, is_mut: bool }', 'Owned(Box<MLIRType>)'], "MLIRType definition"),
    ("MLIROperation", ['Move { source: String, dest: String }', 'Borrow { value: String, lifetime: String, is_mut: bool }', 'Drop { value: String }', 'Call { callee: String, args: Vec<String>, effects: EffectSet }', 'PureCall { callee: String, args: Vec<String> }', 'WithEffect { effects: EffectSet, body: String }', 'Verify { condition: String }', 'VerifiedBlock { body: Vec<MLIROperation> }'], "MLIROperation definition"),
    ("EffectSet", ['PURE', 'IO', 'MEMORY', 'UNKNOWN'], "EffectSet definition"),
]

@dataclass
class AstToMlirCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        guard = f"NOVA_AST_TO_MLIR_TAGS_H"
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_AST_TO_MLIR_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = AstToMlirCompilerGenerator().compile()
