...
CASES: List[Tuple[str, List[str], str]] = [
    ("JITMode", ['Lazy', 'Eager', 'Adaptive'], "JITMode definition"),
    ("OptimizationLevel", ['NoOptimization', 'BasicOptimization', 'StandardOptimization', 'AggressiveOptimization'], "OptimizationLevel definition"),
    ("TargetArch", ['X86_64', 'ARM64', 'RISCV64'], "TargetArch definition"),
    ("CompilationStatus", ['NotCompiled', 'Compiling', 'Compiled', 'Failed'], "CompilationStatus definition"),
    ("OptLevel", ['None', 'Speed', 'SpeedAndSize'], "OptLevel definition"),
]

@dataclass
class JitCompilerGenerator:
    ...
    def emit_c_header(self) -> str:
        ...
        for group_name, variants, doc in self.cases:
            ...
            for var in variants:
                ...
                output += f"#define NOVA_JIT_{group_name.upper()}_{v_clean} {tag}\n"
            output += "\n"
        output += "#endif\n"
        return output
...
def main():
    ...
    args = p.parse_args()
    c = JitCompilerGenerator().compile()
