...
static nova_expr_t *nova_expr_clone(nova_expr_t *expr) {
  ...
  *c = *expr; /* shallow baseline */
  ...
  case EXPR_STR:
    c->data.lit_str =
        expr->data.lit_str ? ast_strdup(expr->data.lit_str) : NULL;
    break;
  case EXPR_IDENT:
    c->data.ident = expr->data.ident ? ast_strdup(expr->data.ident) : NULL;
    break;
  case EXPR_BINARY:
    c->data.binary.op =
        expr->data.binary.op ? ast_strdup(expr->data.binary.op) : NULL;
  ...
  case EXPR_UNARY:
    c->data.unary.op =
        expr->data.unary.op ? ast_strdup(expr->data.unary.op) : NULL;
    c->data.unary.operand = nova_expr_clone(expr->data.unary.operand);
    break;
  default:
...
void nova_expr_free(nova_expr_t *expr) {
  ...
  case EXPR_UNARY:
    ...
    nova_expr_free(expr->data.unary.operand);
    break;
  default:
    break;
  }
...
void nova_stmt_free(nova_stmt_t *stmt) {
  ...
  case STMT_CHECK:
    ...
    if (stmt->data.check_stmt.pattern)
      nova_pattern_free(stmt->data.check_stmt.pattern);
...
/* ── nova_type_clone
 * ───────────────────────────────────────────────────────── Public deep-copy
 * function (declared in ast.h, used by semantic, module_registry, etc.)
 */
...
/* ── nova_stmt_var_decl
 * ────────────────────────────────────────────────────── Declared in ast.h
 * alongside nova_stmt_var().  Both produce STMT_VAR_DECL; nova_stmt_var_decl
 * is the canonical name used by the parser and codegen.
 */
...
nova_expr_t *nova_expr_unary(nova_expr_t *operand, const char *op,
  ...
  expr->data.unary.op = ast_strdup(op);
  return expr;
}
