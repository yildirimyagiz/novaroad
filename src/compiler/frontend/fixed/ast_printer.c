...
static void print_type(nova_type_t *type) {
  ...
  case TYPE_USIZE:   printf("usize");  break;
  case TYPE_STR:     printf("str");    break;
  case TYPE_PTR:     printf("*"); print_type(type->data.ptr.pointee); break;
  case TYPE_PTR_MUT: printf("*mut "); print_type(type->data.ptr.pointee); break;
  case TYPE_POINTER: printf("ptr<"); print_type(type->data.ptr.pointee); printf(">"); break;
  case TYPE_ARRAY:
    printf("["); print_type(type->data.ptr.pointee);
    if (type->data.ptr.size) printf("; %zu", type->data.ptr.size);
    printf("]"); break;
  case TYPE_DATA:    printf("%s", type->data.name ? type->data.name : "?"); break;
  case TYPE_VAR:     printf("%s", type->data.var_name ? type->data.var_name : "?"); break;
  case TYPE_FLOW:
    switch (type->data.flow.flow_kind) {
    case FLOW_SIGNAL: printf("Signal<"); break;
    case FLOW_STREAM: printf("Stream<"); break;
    case FLOW_TASK:   printf("Task<");   break;
    case FLOW_CHAN:   printf("Chan<");   break;
    }
    print_type(type->data.flow.inner);
    printf(">");
    break;
  default: printf("<type:%d>", type->kind); break;
  }
}

static void print_expr(nova_expr_t *expr, int depth) {
  if (!expr) { print_indent(depth); printf("<null_expr>\n"); return; }
  print_indent(depth);
  switch (expr->kind) {
  case EXPR_INT:    printf("int(%lld)\n",  (long long)expr->data.lit_int);  break;
  case EXPR_FLOAT:  printf("float(%g)\n",  expr->data.lit_float);           break;
  case EXPR_BOOL:   printf("bool(%s)\n",   expr->data.lit_bool ? "true" : "false"); break;
  case EXPR_STR:    printf("str(\"%s\")\n", expr->data.lit_str ? expr->data.lit_str : ""); break;
  case EXPR_IDENT:  printf("ident(%s)\n",  expr->data.ident ? expr->data.ident : "?"); break;
  case EXPR_BINARY:
...
static void print_stmt(nova_stmt_t *stmt, int depth) {
  ...
  case STMT_VAR_DECL:
    print_indent(depth);
    printf("let %s: ", stmt->data.var_decl.name ? stmt->data.var_decl.name : "?");
  ...
  case STMT_FOR:
    ...
    break;
  default:
    print_indent(depth); printf("stmt(%d)\n", stmt->kind);
    break;
  }
}

void nova_program_print(nova_program_t *program) {
  if (!program) { printf("<null program>\n"); return; }
