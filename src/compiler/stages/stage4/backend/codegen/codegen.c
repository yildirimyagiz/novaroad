...
/**
 * try_fold_binary: Try to fold a binary expression with two compile-time operands.
 * Returns true and sets *result if both sides are constants and op is arithmetic.
 */
...
static loop_context_t *loop_context_create(int loop_start, loop_context_t *enclosing)
{
...
static void loop_context_add_break(loop_context_t *ctx, int offset)
{
    if (ctx->break_count >= ctx->break_capacity) {
        ...
        ctx->break_jumps = realloc(ctx->break_jumps, sizeof(int) * ctx->break_capacity);
    }
...
static void loop_context_add_continue(loop_context_t *ctx, int offset)
{
    if (ctx->continue_count >= ctx->continue_capacity) {
        ...
        ctx->continue_jumps = realloc(ctx->continue_jumps, sizeof(int) * ctx->continue_capacity);
    }
...
static void codegen_register_enum(nova_codegen_t *codegen, nova_stmt_t *stmt)
{
    if (codegen->enum_count >= codegen->enum_capacity) {
        codegen->enum_capacity = codegen->enum_capacity == 0 ? 4 : codegen->enum_capacity * 2;
        codegen->enums = realloc(codegen->enums, sizeof(enum_entry_t) * codegen->enum_capacity);
    }
    ...
    for (int i = 0; i < e->variant_count; i++) {
        ...
        e->variants[i].field_count = (int) stmt->data.enum_decl.variants[i].field_count;
    }
}

static bool lookup_variant_tag(nova_codegen_t *codegen, const char *variant_name, int *tag,
                               int *field_count)
{
...
static bool generate_pattern_check(nova_codegen_t *codegen, nova_pattern_t *pat)
{
    if (!pat)
        return false;
    ...
    case PATTERN_IDENT: {
        ...
        if (ident && lookup_variant_tag(codegen, ident, &tag, &field_count)) {
            ...
            int constant_idx = chunk_add_constant(codegen->chunk, value_number((double) tag));
            ...
            codegen->chunk->code[false_jump] = (uint8_t) ((false_target - false_jump - 2) >> 8);
            ...
            codegen->chunk->code[exit_jump] = (uint8_t) ((exit_target - exit_jump - 2) >> 8);
            codegen->chunk->code[exit_jump + 1] = (uint8_t) ((exit_target - exit_jump - 2) & 0xff);
    ...
    case PATTERN_ENUM: {
        ...
        if (!lookup_variant_tag(codegen, pat->data.variant.name, &tag, &expected_fields)) {
        ...
        chunk_write_opcode(codegen->chunk, OP_GET_TAG, pat->span.line);
        chunk_write_opcode(codegen->chunk, OP_CONSTANT, pat->span.line);
        int constant = chunk_add_constant(codegen->chunk, value_number((double) tag));
        ...
        int false_target = codegen->chunk->count;
        codegen->chunk->code[false_jump] = (uint8_t) ((false_target - false_jump - 2) >> 8);
        codegen->chunk->code[false_jump + 1] = (uint8_t) ((false_target - false_jump - 2) & 0xff);
        chunk_write_opcode(codegen->chunk, OP_POP, pat->span.line);
        chunk_write_opcode(codegen->chunk, OP_FALSE, pat->span.line);
...
static symbol_t *codegen_lookup_variable(nova_codegen_t *codegen, const char *name)
{
    // 1. Check our own local table
    symbol_t *curr = codegen->locals;
    while (curr) {
...
static void codegen_begin_scope(nova_codegen_t *codegen)
{
    if (codegen->scope_depth < 64) {
        codegen->local_index_stack[codegen->scope_depth] = codegen->next_local_index;
    }
    codegen->scope_depth++;
}
...
static void codegen_end_scope(nova_codegen_t *codegen, int line)
{
    ...
    symbol_t **pp = &codegen->locals;
    while (*pp) {
        symbol_t *curr = *pp;
        // Remove locals that were added in this scope (their index is >= prev_local_index)
...
static bool generate_expression(nova_codegen_t *codegen, nova_expr_t *expr)
{
    ...
    case EXPR_STR:
        ...
        chunk_write_opcode(codegen->chunk, OP_CONSTANT, expr->span.line);
        constant = chunk_add_constant(codegen->chunk, value_string(expr->data.lit_str));
        chunk_write(codegen->chunk, (uint8_t) constant, expr->span.line);
        break;
    ...
    case EXPR_CALL: {
        ...
        if (expr->data.call.func->kind == EXPR_NAMESPACED_ACCESS) {
            int tag, fields;
            if (lookup_variant_tag(codegen, expr->data.call.func->data.namespaced_access.member,
                                   &tag, &fields)) {
                ...
                chunk_write(codegen->chunk, (uint8_t) expr->data.call.arg_count, expr->span.line);
                break;
            }
        }
        ...
        // Lookup function by name (callee must be identifier)
        if (expr->data.call.func->kind != EXPR_IDENT) {
            codegen->error_message = strdup("Function call requires identifier");
            return false;
        }

        const char *fn_name = expr->data.call.func->data.ident;

        // Handle builtin functions (arguments already emitted above)
        if (strcmp(fn_name, "println") == 0 || strcmp(fn_name, "print") == 0) {
            // Emit OP_PRINT opcode
            chunk_write_opcode(codegen->chunk, OP_PRINT, expr->span.line);
            break;
        }
        /* String builtins — resolved at link time via stdlib */
        if (strcmp(fn_name, "String_len") == 0 || strcmp(fn_name, "String_from") == 0 ||
            strcmp(fn_name, "String_from_literal") == 0 || strcmp(fn_name, "assert") == 0) {
        ...
        if (target_vpu != 0) {
            ...
            chunk_write(codegen->chunk, (fn_offset >> 8) & 0xff, expr->span.line); // High byte
            chunk_write(codegen->chunk, fn_offset & 0xff, expr->span.line);        // Low byte
            chunk_write(codegen->chunk, (uint8_t) expr->data.call.arg_count, expr->span.line);
        }
    ...
    case EXPR_IDENT: {
        // Lookup variable
        symbol_t *symbol = codegen_lookup_variable(codegen, expr->data.ident);
    ...
    case EXPR_ASSIGN: {
        ...
        // Lookup variable
        symbol_t *symbol = codegen_lookup_variable(codegen, expr->data.assign.name);
    ...
    case EXPR_ADDR_OF: {
        ...
        if (expr->data.addr_of->kind != EXPR_IDENT) {
            codegen->error_message = strdup("Can only take address of variables (for now)");
            return false;
        }

        symbol_t *symbol = codegen_lookup_variable(codegen, expr->data.addr_of->data.ident);
        if (!symbol) {
            char error[256];
            ...
            if (target_unit) {
                ...
                if (to_dim) {
                    ...
                    if (to_scale != 1.0 && !isnan(to_scale) && to_scale != 0.0) {
                        ...
                        int inv_const =
                            chunk_add_constant(codegen->chunk, value_number(1.0 / to_scale));
    ...
    case EXPR_ARRAY_LIT: {
        ...
        chunk_write(codegen->chunk, (uint8_t) expr->data.array_lit.count, expr->span.line);
        break;
    }
    ...
    case EXPR_NAMESPACED_ACCESS: {
        ...
        int tag, fields;
        if (lookup_variant_tag(codegen, expr->data.namespaced_access.member, &tag, &fields)) {
            if (fields == 0) {
                chunk_write_opcode(codegen->chunk, OP_ENUM, expr->span.line);
                chunk_write(codegen->chunk, (uint8_t) tag, expr->span.line);
        ...
        // General namespaced access not fully implemented
        codegen->error_message = strdup("Namespaced access not fully implemented");
        return false;
    }
    ...
    case EXPR_MATCH: {
        ...
        for (size_t i = 0; i < expr->data.match.arm_count; i++) {
            ...
            chunk_write_opcode(codegen->chunk, OP_JUMP_IF_FALSE, arm->pattern->span.line);
            int false_jump = codegen->chunk->count;
            ...
            int false_target = codegen->chunk->count;
            codegen->chunk->code[false_jump] = ((false_target - false_jump - 2) >> 8) & 0xff;
            codegen->chunk->code[false_jump + 1] = (false_target - false_jump - 2) & 0xff;
        }
        ...
        for (int i = 0; i < exit_jump_count; i++) {
            ...
            codegen->chunk->code[jump_off] = ((exit_target - jump_off - 2) >> 8) & 0xff;
    ...
    case EXPR_ENUM_VARIANT: {
        ...
        if (name && lookup_variant_tag(codegen, name, &tag, &fields)) {
            ...
            chunk_write_opcode(codegen->chunk, OP_CONSTANT, expr->span.line);
            int constant_idx = chunk_add_constant(codegen->chunk, value_number(variant_id));
...
static bool generate_statement(nova_codegen_t *codegen, nova_stmt_t *stmt)
{
    ...
    case STMT_CHECK: {
        ...
        codegen->chunk->code[then_jump] = ((then_target - then_jump - 2) >> 8) & 0xff;
        ...
        codegen->chunk->code[else_jump] = ((else_target - else_jump - 2) >> 8) & 0xff;
    ...
    case STMT_FN: {
        ...
        if (codegen->function_count >= codegen->function_capacity) {
            ...
            codegen->functions =
                realloc(codegen->functions, sizeof(function_entry_t) * codegen->function_capacity);
        }
    ...
    case STMT_CONTINUE: {
        ...
        int loop_offset = (codegen->chunk->count + 2) - codegen->current_loop->loop_start;
    ...
    case STMT_VAR_DECL: {
        ...
        symbol_t *symbol =
            nova_semantic_lookup_variable(codegen->semantic, stmt->data.var_decl.name);
    ...
    case STMT_STRUCT_DECL:
        // Struct declarations don't generate bytecode in the simple VM
        // but might need registration if we implement struct support later.
        break;
    ...
    case STMT_WHILE: {
        ...
        loop_context_t *loop_ctx = loop_context_create(loop_start, codegen->current_loop);
        ...
        // Patch exit jump
        int exit_target = codegen->chunk->count;
        codegen->chunk->code[exit_jump] = ((exit_target - exit_jump - 2) >> 8) & 0xff;
    ...
    case STMT_FOR: {
        ...
        chunk_write_opcode(codegen->chunk, OP_SET_LOCAL, stmt->span.line);
        chunk_write(codegen->chunk, (uint8_t) iter_index, stmt->span.line);
        chunk_write_opcode(codegen->chunk, OP_POP, stmt->span.line); // Pop assignment result
...
static bool generate_program(nova_codegen_t *codegen, nova_program_t *program)
{
    ...
    for (size_t i = 0; i < program->declaration_count; i++) {
        ...
        if (((nova_stmt_t *) decl->data)) {
            ...
            // decl->data)->kind); Generate any statement type (STMT_VAR, STMT_EXPR, etc.)
...
bool nova_codegen_generate_from_ast(nova_codegen_t *codegen, nova_program_t *ast);

nova_codegen_t *nova_codegen_create(nova_backend_type_t backend)
{
    (void) backend; // Unused for now - always use VM backend
...
void nova_codegen_set_semantic(nova_codegen_t *codegen, nova_semantic_t *semantic)
{
