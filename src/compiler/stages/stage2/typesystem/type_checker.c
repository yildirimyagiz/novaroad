...
/* symbol_table_t already defined in ast.h */
...
static void symbol_table_destroy(symbol_table_t *table) {
  symbol_t *cur = table->symbols;
  while (cur) {
...
/** * symbol_table_add: Adds a symbol.
...
static void symbol_table_update_type(symbol_table_t *table, const char *name,
                                     const nova_type_t *new_type) {
  symbol_t *sym = symbol_table_lookup(table, name);
  if (sym) {
 ...
 */
static void begin_scope(symbol_table_t *table) {
 ...
 */
static void end_scope(symbol_table_t *table) {
  if (table->scope_depth == 0)
    return;

  /* Remove and free symbols at current depth */
  symbol_t **pp = &table->symbols;
  while (*pp) {
    symbol_t *cur = *pp;
...
static void semantic_set_error(nova_semantic_t *semantic, const char *fmt,
                               ...) {
  free((void *)semantic->error_message); /* [Fix 10] */
  char buf[512];
  va_list ap;
  va_start(ap, fmt);
...
static bool types_compatible(const nova_type_t *a, const nova_type_t *b) {
  ...
  if (a->kind == b->kind) {
    ...
    /* For GENERIC types: name and args must match */
    if (a->kind == TYPE_GENERIC) {
      if (strcmp(a->data.generic.name, b->data.generic.name) != 0)
        return false;
      if (a->data.generic.arg_count != b->data.generic.arg_count)
        return false;
      for (size_t i = 0; i < a->data.generic.arg_count; i++) {
        if (!types_compatible(a->data.generic.args[i], b->data.generic.args[i]))
          return false;
      }
      return true;
    }
    /* For TUPLE types: element counts and types must match */
    if (a->kind == TYPE_TUPLE) {
      if (a->data.tuple.count != b->data.tuple.count)
        return false;
      for (size_t i = 0; i < a->data.tuple.count; i++) {
        if (!types_compatible(a->data.tuple.elements[i],
                              b->data.tuple.elements[i]))
          return false;
      }
      return true;
    }
    /* For VAR types: just compare names */
    if (a->kind == TYPE_VAR) {
  ...
  if ((a->kind == TYPE_STR && b->kind == TYPE_DATA) ||
    ...
    return true;
  /* VARIANT compatible if names match or something (simplified) */
  return false;
}
...
static nova_type_t *analyze_expression(nova_semantic_t *semantic,
  ...
  case EXPR_UNIT_LITERAL:
    /* Use the unit string to create a QTY type */
    expr->type = nova_type_qty(nova_type_f64(), expr->data.unit_literal.unit);
    return nova_type_clone(expr->type);

  case EXPR_IDENT: {
    symbol_t *sym = symbol_table_lookup(table, expr->data.ident);
    if (!sym) {
  ...
  case EXPR_ASSIGN: {
    symbol_t *sym = symbol_table_lookup(table, expr->data.assign.name);
    if (!sym) {
  ...
  case EXPR_FIELD_ACCESS: {
    ...
    nova_type_free(base);
    /* TODO: Struct tip bilgisi gerekiyor */
    expr->type = nova_type_i64();
    return nova_type_clone(expr->type);
  }
  ...
  case EXPR_ENUM_VARIANT: {
    ...
    symbol_t *enum_sym =
        symbol_table_lookup(table, expr->data.enum_variant.enum_name);
    if (!enum_sym) {
  ...
  case EXPR_BLOCK: {
    /* Analyze the block as a statement.
     * Pass NULL for fn_ctx since analyze_expression doesn't have it yet. */
    if (!analyze_statement_with_context(semantic, expr->data.block, table,
                                        NULL))
      return NULL;
    /* Block expressions in Nova v10 evaluate to the last expression's type.
     * For now, simplify and treat as i64 or let tail expression logic handle
     * it. */
...
static void register_pattern_bindings(nova_semantic_t *semantic,
  ...
  case PATTERN_IDENT:
    if (pat->data.ident) {
      ...
      symbol_table_add(table, pat->data.ident, t, true);
      nova_type_free(t);
    }
    break;
  case PATTERN_ENUM:
...
static bool analyze_statement_with_context(nova_semantic_t *semantic,
  ...
  case STMT_VAR_DECL: {
     ...
     *   - symbol_table_add always clones → we can always free
     */
    ...
    if (var_type) {
      symbol_table_add(table, stmt->data.var_decl.name, var_type, true);
    } else {
      nova_type_t *def = nova_type_i64();
      symbol_table_add(table, stmt->data.var_decl.name, def, true);
      nova_type_free(def);
    }
  ...
  case STMT_TRY_CATCH: {
    /* Analyze try block */
    if (!analyze_statement_with_context(
            semantic, stmt->data.try_catch_stmt.try_block, table, fn_ctx))
      return false;
    /* Analyze catch block (if it exists) */
    if (stmt->data.try_catch_stmt.catch_block) {
      begin_scope(table);
      if (stmt->data.try_catch_stmt.exception_name) {
        /* Exception is normally an Error/str type */
        symbol_table_add(table, stmt->data.try_catch_stmt.exception_name,
                         nova_type_str(), true); // Simplified
      }
      bool res = analyze_statement_with_context(
          semantic, stmt->data.try_catch_stmt.catch_block, table, fn_ctx);
      end_scope(table);
      if (!res)
        return false;
    }
  ...
  case STMT_FOR: {
    ...
    symbol_table_add(table, stmt->data.for_stmt.binding, it, false);
    nova_type_free(it);
  ...
  case STMT_FN: {
    ...
    if (stmt->data.fn_stmt.name && table->scope_depth > 0) {
      ...
      symbol_table_add(table, stmt->data.fn_stmt.name, fn_type, false);
    ...
    for (size_t i = 0; i < stmt->data.fn_stmt.param_count; i++) {
      ...
      if (p->type) {
        symbol_table_add(table, p->name, p->type, true);
      } else {
        nova_type_t *def = nova_type_i32();
        symbol_table_add(table, p->name, def, true);
        nova_type_free(def);
      }
    }
    ...
    if (ok) {
      ...
      if (stmt->data.fn_stmt.name && table->scope_depth == 1) {
        ...
        symbol_t *orig_sym =
            symbol_table_lookup(table, stmt->data.fn_stmt.name);
        if (orig_sym && orig_sym->type && orig_sym->type->kind == TYPE_FN) {
          ...
          symbol_table_update_type(table, stmt->data.fn_stmt.name, new_fn_type);
          nova_type_free(new_fn_type);
        } else {
          symbol_table_update_type(table, stmt->data.fn_stmt.name, fn_type);
        }
  ...
  case STMT_BREAK:
  case STMT_CONTINUE:
    /* TODO: Döngü içinde olup olmadığı kontrolü */
    return true;
  ...
  case STMT_IMPORT: {
    ...
    if (strcmp(module_name, "physics") != 0) {
      ...
      /* symbol_table_add wrapper for the registry callback */
      typedef void (*add_fn_t)(void *, const char *, const nova_type_t *, bool);
      add_fn_t add_cb = (add_fn_t)(void *)symbol_table_add;
    ...
    if (strcmp(module_name, "physics") == 0) {
      ...
      for (const nova_physics_constant_t *pc = nova_physics_constants; pc->name;
         ...
         * immutable */
        symbol_table_add(table, pc->name, const_type, false);
      }
      return true;
    }
...
static bool analyze_program(nova_semantic_t *semantic,
  ...
  symbol_table_t *global_table = symbol_table_create();
  if (!global_table)
    return false;
  ...
  /* Built-in symbols */
  {
    ...
    symbol_table_add(global_table, "assert", vt, false);
    symbol_table_add(global_table, "assert_eq", vt, false);
    symbol_table_add(global_table, "assert_ne", vt, false);
    symbol_table_add(global_table, "assert_approx_eq", vt, false);
    symbol_table_add(global_table, "panic", vt, false);
    symbol_table_add(global_table, "eprintln", vt, false);
    symbol_table_add(global_table, "eprint", vt, false);
  ...
  /* String builtin functions */
  {
    ...
    symbol_table_add(global_table, "String_len", i64_type, false);
    symbol_table_add(global_table, "String_from", i64_type, false);
    symbol_table_add(global_table, "String_from_literal", i64_type, false);
    symbol_table_add(global_table, "String_concat", i64_type, false);
  ...
  /* Built-in Enum Variants (Result, Option) for bootstrap */
  {
    ...
    symbol_table_add(global_table, "Ok", rt, false);
    symbol_table_add(global_table, "Err", rt, false);
    nova_type_free(rt);

    nova_type_t *ot = nova_type_data("Option");
    symbol_table_add(global_table, "Some", ot, false);
    symbol_table_add(global_table, "None", ot, false);
    nova_type_free(ot);
  }
  ...
  for (size_t i = 0; i < program->declaration_count; i++) {
    ...
    if (stmt->kind == STMT_ENUM_DECL && stmt->data.enum_decl.name) {
      ...
      symbol_table_add(global_table, stmt->data.enum_decl.name, enum_type,
                       false);
      ...
      for (size_t j = 0; j < stmt->data.enum_decl.variant_count; j++) {
        ...
        symbol_table_add(global_table, variant_name, variant_type, false);
      ...
      symbol_table_add(global_table, stmt->data.fn_stmt.name, fn_type, false);
      nova_type_free(fn_type);
    }
  }
...
void nova_semantic_destroy(nova_semantic_t *semantic) {
  if (!semantic)
    return;
  free((void *)semantic->error_message);
  if (semantic->symbol_table)
    symbol_table_destroy(semantic->symbol_table);
  free(semantic);
}

symbol_t *nova_semantic_lookup_variable(nova_semantic_t *semantic,
                                        const char *name) {
  if (!semantic || !semantic->symbol_table)
    return NULL;
  return symbol_table_lookup(semantic->symbol_table, name);
}
