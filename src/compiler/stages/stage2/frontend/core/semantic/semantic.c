...
static bool type_is_numeric(const nova_type_t *t) {
  return t &&
         (t->kind == SEM_TYPE_I32 || t->kind == SEM_TYPE_I64 || t->kind == SEM_TYPE_F32 ||
          t->kind == SEM_TYPE_F64 || t->kind == SEM_TYPE_U8 || t->kind == SEM_TYPE_USIZE ||
          t->kind == SEM_TYPE_BOOL);
}

static bool type_is_float(const nova_type_t *t) {
  return t && (t->kind == SEM_TYPE_F32 || t->kind == SEM_TYPE_F64);
}

static bool type_is_pointer(const nova_type_t *t) {
  return t && (t->kind == SEM_TYPE_PTR || t->kind == SEM_TYPE_PTR_MUT ||
               t->kind == SEM_TYPE_POINTER);
}
...
static bool types_compatible(const nova_type_t *a, const nova_type_t *b) {
  ...
  if (a->kind == SEM_TYPE_DATA && a->data.name && strcmp(a->data.name, "any") == 0)
    return true;
  if (b->kind == SEM_TYPE_DATA && b->data.name && strcmp(b->data.name, "any") == 0)
    return true;

  if (a->kind == b->kind) {
    /* For DATA types, check names match (or accept if either name is NULL) */
    if (a->kind == SEM_TYPE_DATA) {
   ...
   */
  if (a->kind == SEM_TYPE_QTY && type_is_numeric(b))
    return types_compatible(a->data.qty.inner_type, b);
  if (b->kind == SEM_TYPE_QTY && type_is_numeric(a))
  ...
  if ((a->kind == SEM_TYPE_DATA && type_is_pointer(b)) ||
      (b->kind == SEM_TYPE_DATA && type_is_pointer(a)))
    return true;
  /* STR is compatible with pointer types (string literals to *String) */
  if ((a->kind == SEM_TYPE_STR && type_is_pointer(b)) ||
      (b->kind == SEM_TYPE_STR && type_is_pointer(a)))
    return true;
  /* STR is compatible with DATA (user-defined 'shape String' vs builtin str) */
  if ((a->kind == SEM_TYPE_STR && b->kind == SEM_TYPE_DATA) ||
      (a->kind == SEM_TYPE_DATA && b->kind == SEM_TYPE_STR))
    return true;
  return false;
}
...
static nova_dimension_t *qty_get_or_parse_dim(nova_type_t *qty_type) {
  if (!qty_type || qty_type->kind != SEM_TYPE_QTY)
    return NULL;
...
static nova_type_t *analyze_expression(nova_semantic_t *semantic,
  ...
  case EXPR_BINARY: {
    ...
    if (strcmp(op, "in") == 0) {
      ...
      if (!lt)
        return NULL;
      bool lt_is_qty = (lt->kind == SEM_TYPE_QTY);
    ...
    bool lt_is_qty = (lt->kind == SEM_TYPE_QTY);
    bool rt_is_qty = (rt->kind == SEM_TYPE_QTY);
  ...
  case EXPR_DEREF: {
    ...
    /* Accept SEM_TYPE_POINTER, SEM_TYPE_PTR (shared borrow) and SEM_TYPE_PTR_MUT (mutable
     * borrow) */
    if (pt->kind != SEM_TYPE_POINTER && pt->kind != SEM_TYPE_PTR &&
        pt->kind != SEM_TYPE_PTR_MUT) {
  ...
  case EXPR_CALL: {
    ...
    /* Check if function type is SEM_TYPE_FN and validate arguments */
    if (func_sym && func_sym->type && func_sym->type->kind == SEM_TYPE_FN) {
    ...
    /* Old path for non-SEM_TYPE_FN functions (builtins, etc.) */
    nova_type_free(ct);
  ...
  case EXPR_STRUCT_INIT: {
    /* Struct initializer: StructName { ... } returns SEM_TYPE_DATA */
  ...
  case EXPR_INDEX: {
    ...
    if (ot->kind != SEM_TYPE_ARRAY && ot->kind != SEM_TYPE_POINTER &&
        ot->kind != SEM_TYPE_STR) {
    ...
    nova_type_t *res;
    if (ot->kind == SEM_TYPE_ARRAY) {
      res = nova_type_clone(ot->data.ptr.pointee);
    } else if (ot->kind == SEM_TYPE_POINTER) {
      res = nova_type_clone(ot->data.ptr.pointee);
    } else {
  ...
  case EXPR_ENUM_VARIANT: {
    ...
    /* Verify it's a SEM_TYPE_DATA (enum/struct) */
    if (enum_sym->type->kind != SEM_TYPE_DATA) {
...
static bool analyze_statement_with_context(nova_semantic_t *semantic,
  ...
  case STMT_CHECK: {
    ...
    if (!ct)
      return false;
    if (!stmt->data.check_stmt.pattern && ct->kind != SEM_TYPE_BOOL) {
    ...
    if (!return_val) {
      if (fn_ctx) {
        ...
        if (fn_ctx->declared_return &&
            fn_ctx->declared_return->kind != SEM_TYPE_VOID) {
          semantic_set_error(semantic,
        ...
        if (!fn_ctx->declared_return && fn_ctx->inferred_return &&
            fn_ctx->inferred_return->kind != SEM_TYPE_VOID) {
          semantic_set_error(
  ...
  case STMT_FN: {
    ...
    if (stmt->data.fn_stmt.name && table->scope_depth > 0) {
      /* Build SEM_TYPE_FN with parameter types */
    ...
    if (ok) {
      ...
      if (effective_return && effective_return->kind != SEM_TYPE_VOID) {
      ...
      if (stmt->data.fn_stmt.name && table->scope_depth == 1) {
        ...
        if (orig_sym && orig_sym->type && orig_sym->type->kind == SEM_TYPE_FN) {
          nova_type_t *new_fn_type = malloc(sizeof(nova_type_t));
          new_fn_type->kind = SEM_TYPE_FN;
  ...
  for (size_t i = 0; i < program->declaration_count; i++) {
    ...
    if (stmt->kind == STMT_ENUM_DECL && stmt->data.enum_decl.name) {
      ...
      for (size_t j = 0; j < stmt->data.enum_decl.variant_count; j++) {
        ...
        variant_type->kind = SEM_TYPE_DATA;
