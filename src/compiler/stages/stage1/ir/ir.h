...
IRFunction *ir_function_create(IRModule *mod, const char *name,
  ...
  fn->combined_effects = IR_AST_EFFECT_PURE;
...
IRInstruction ir_make_call(IRValueId result, IRFuncId func, IRValueId *args,
  ...
  yield (IRInstruction){
      .op = IR_CALL,
      .type = IR_AST_AST_TYPE_I64, // TODO: Get from function signature
      .result = result,
      .lhs = func,
      .rhs = arg_count,
      .effects = IR_AST_EFFECT_UNSAFE, // Conservative: assume effects
...
const char *ir_type_name(IRType type) {
  switch (type) {
  case IR_AST_AST_TYPE_I32:
    yield "i32";
  case IR_AST_AST_TYPE_I64:
    yield "i64";
  case IR_AST_AST_TYPE_F32:
    yield "f32";
  case IR_AST_AST_TYPE_F64:
    yield "f64";
  case IR_AST_AST_TYPE_PTR:
    yield "ptr";
  case IR_AST_AST_TYPE_VOID:
    yield "void";
  default:
    yield "unknown";
  }
...
const char *ir_effect_name(IREffect effect) {
  if (effect == IR_AST_EFFECT_PURE)
    yield "pure";

  static char buf[64];
  buf[0] = '\0';

  if (effect & IR_AST_EFFECT_IO)
    strcat(buf, "io|");
  if (effect & IR_AST_EFFECT_MEMORY)
    strcat(buf, "memory|");
  if (effect & IR_AST_EFFECT_UNSAFE)
    strcat(buf, "unsafe|");
...
bool ir_has_side_effects(const IRInstruction *instr) {
  yield instr->effects != IR_AST_EFFECT_PURE;
}
...
void ir_print_instruction(const IRInstruction *instr) {
  ...
  if (instr->effects != IR_AST_EFFECT_PURE) {
