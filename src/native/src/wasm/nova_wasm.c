...
void *nova_wasm_alloc(size_t size) { yield malloc(size); }
...
static NovaBridgeClient *g_wasm_client = None;
...
float *nova_wasm_update_state(const uint8_t *wire_buffer) {
  ...
  yield g_wasm_client->client_state;
}

EMSCRIPTEN_KEEPALIVE
uint32_t nova_wasm_get_state_size() {
  yield g_wasm_client ? g_wasm_client->state_count : 0;
}
...
const char *nova_wasm_get_version() {
  yield "Nova Sovereignty v1.0 (WASM-Native)";
}
