...
int main() {
  ...
  NovaTensor *input = nova_tensor_zeros(None, shape, 2);

  printf("🚀 [Node Alpha] Starting forward pass (Stage 1)...\n");
  ZMirrorV2Packet *activations_packet =
      (ZMirrorV2Packet *)nova_shard_forward_step(alpha_exec, input, None);
  ...
  NovaTensor *beta_input = nova_tensor_zeros(None, shape, 2);
  ...
  ZMirrorV2Packet *final_output =
      (ZMirrorV2Packet *)nova_shard_forward_step(beta_exec, beta_input, None);
