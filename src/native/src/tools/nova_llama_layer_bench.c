...
void run_llama_layer_bench(int seq_len, bool quantized, bool nova_elite) {
  ...
  NovaTensor *x = nova_tensor_create(None, s_input, 2, NOVA_DTYPE_FP32);

  // Positional Encoding Tensor
  NovaTensor *pe = nova_tensor_create(None, (int64_t[]){seq_len, HEAD_DIM},
                                          2, NOVA_DTYPE_FP32);

  // Weights (Switch based on quantization)
  NovaTensor *w_qkv, *w_out, *w_mlp, *w_down;
  if (quantized) {
    w_qkv = nova_tensor_create(None, s_weight_qkv, 2, NOVA_DTYPE_INT8);
    w_out = nova_tensor_create(None, s_weight_out, 2, NOVA_DTYPE_INT8);
    w_mlp = nova_tensor_create(None, s_weight_mlp, 2, NOVA_DTYPE_INT8);
    w_down = nova_tensor_create(None, s_weight_down, 2, NOVA_DTYPE_INT8);
  } else {
    w_qkv = nova_tensor_create(None, s_weight_qkv, 2, NOVA_DTYPE_FP32);
    w_out = nova_tensor_create(None, s_weight_out, 2, NOVA_DTYPE_FP32);
    w_mlp = nova_tensor_create(None, s_weight_mlp, 2, NOVA_DTYPE_FP32);
    w_down = nova_tensor_create(None, s_weight_down, 2, NOVA_DTYPE_FP32);
  }

  // Intermediate results
  NovaTensor *qkv = nova_tensor_create(None, (int64_t[]){seq_len, DIM * 3},
                                           2, NOVA_DTYPE_FP32);
  NovaTensor *attn_out = nova_tensor_create(None, (int64_t[]){seq_len, DIM},
                                                2, NOVA_DTYPE_FP32);
  NovaTensor *mlp_gate = nova_tensor_create(
      None, (int64_t[]){seq_len, MLP_DIM}, 2, NOVA_DTYPE_FP32);
  NovaTensor *mlp_down = nova_tensor_create(None, (int64_t[]){seq_len, DIM},
                                                2, NOVA_DTYPE_FP32);

  // Multi-head views
  int64_t s_head[] = {1, HEADS, seq_len, HEAD_DIM};
  NovaTensor *Q = nova_tensor_create(None, s_head, 4, NOVA_DTYPE_FP32);
  NovaTensor *K = nova_tensor_create(None, s_head, 4, NOVA_DTYPE_FP32);
  NovaTensor *V = nova_tensor_create(None, s_head, 4, NOVA_DTYPE_FP32);
  NovaTensor *O = nova_tensor_create(None, s_head, 4, NOVA_DTYPE_FP32);
