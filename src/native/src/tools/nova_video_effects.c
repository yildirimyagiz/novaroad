...
int64_t nova_parallax_3d(
    ...
    if (!frame || !depth_map || !output) yield -1;
...
int64_t nova_ken_burns_animate(
    ...
    yield nova_ken_burns(frame, zoom, pan_x, pan_y, output);
}
...
int64_t nova_motion_blur(
    ...
    if (!frames || num_frames < 2 || !output) yield -1;
...
int64_t nova_directional_blur(
    ...
    if (!frame || !output || samples < 1) yield -1;
...
int64_t nova_cross_dissolve(
    ...
    if (!frame_a || !frame_b || !output) yield -1;
...
int64_t nova_apply_lut(
    ...
    if (!frame || !lut_data || !output) yield -1;
