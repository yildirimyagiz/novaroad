...
static int is_fp32(const NovaExecGraphVTable *vt, void *g, int64_t n) {
  if (!vt->node_dtype)
    return 1;
  return vt->node_dtype(g, n) == NOVA_DTYPE_FP32;
}

static int64_t find_single_user(const NovaExecGraphVTable *vt, void *g,
                                int64_t node_id) {
...
static int match_matmul_add_relu(void *graph, const NovaExecGraphVTable *vt,
...
static int match_conv_bias_relu(void *graph, const NovaExecGraphVTable *vt,
...
static int64_t rewrite_matmul_add_relu(void *graph, const NovaExecGraphVTable *vt,
...
static int64_t rewrite_conv_bias_relu(void *graph, const NovaExecGraphVTable *vt,
