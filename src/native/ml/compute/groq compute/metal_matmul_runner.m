...
- (instancetype)init {
  ...
  if (self) {
    ...
    NSError *error = nil;
    NSString *metalPath = [[NSBundle mainBundle] pathForResource:@"matmul"
                                                          ofType:@"metallib"];
    if (!metalPath) {
      metalPath = @"/Users/yldyagz/nova/native/src/compute/groq "
                  @"compute/matmul.metallib";
    }
    ...
    id<MTLFunction> functionFP32 =
        [self.library newFunctionWithName:@"matmul_final_ultra_fp32"];
    self.pipelineFP32 =
        [self.device newComputePipelineStateWithFunction:functionFP32
                                                   error:&error];

    id<MTLFunction> functionTiled = [self.library
        newFunctionWithName:@"matmul_gpu_tiled_fp32"]; // Keep for comparison
    self.pipelineTiled =
        [self.device newComputePipelineStateWithFunction:functionTiled
                                                   error:&error];

    id<MTLFunction> functionFP16 =
        [self.library newFunctionWithName:@"matmul_final_ultra_fp16"];
    self.pipelineFP16 =
        [self.device newComputePipelineStateWithFunction:functionFP16
                                                   error:&error];

    id<MTLFunction> functionINT8 =
        [self.library newFunctionWithName:@"matmul_final_ultra_int8"];
    self.pipelineINT8 =
        [self.device newComputePipelineStateWithFunction:functionINT8
                                                   error:&error];
  }
  return self;
}
...
- (void)runGPUFinalUltra:(int)size {
  ...
  id<MTLBuffer> bufferA =
      [self.device newBufferWithLength:numElements * sizeof(float)
  ...
  int errors = 0;
  for (int i = 0; i < numElements; i++) {
    if (fabs(cData[i] - size) > 1e-3)
      errors++;
  }

  printf("Metal GPU Final Ultra FP32 Matmul %dx%d: %.6f s, %.2f GFLOPS, "
         "Errors: %d/%d\n",
...
printf(
    "Metal GPU Tiled FP32 Matmul %dx%d: %.6f s, %.2f GFLOPS, Errors: %d/%d\n",
    size, size, time, gflops, errors, numElements);
}
