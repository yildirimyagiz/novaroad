...
NovaExecGraphNode *nova_exec_graph_add_input(NovaExecGraph *graph, NovaTensor *t,
  ...
  if (graph->node_count >= graph->node_capacity) {
    ...
    graph->nodes =
        realloc(graph->nodes, graph->node_capacity * sizeof(NovaExecGraphNode *));
  }
...
static int64_t g_node_count(void *g) { return ((NovaExecGraph *)g)->node_count; }
...
static int64_t g_node_create(void *g, int32_t op, const int64_t *inputs,
  ...
  NovaExecGraphNode **input_ptrs = malloc(n_inputs * sizeof(NovaExecGraphNode *));
  ...
  NovaExecGraphNode *new_node = nova_exec_graph_add_op(
      graph, (NovaExecGraphOpType)op, input_ptrs, real_input_count, "FusedNode");
