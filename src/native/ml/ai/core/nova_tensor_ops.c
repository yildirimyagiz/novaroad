...
NovaTensor *nova_op_transpose(NovaTensor *x, int dim0, int dim1) {
  /* Create new shape with transposed dimensions */
  int64_t *new_shape = malloc(x->ndim * sizeof(int64_t));
  ...
  NovaTensor *res = nova_tensor_create(x->context, new_shape, x->ndim, x->dtype);
  
  /* Perform transpose for 2D case (common for linear layers) */
  if (x->ndim == 2) {
    ...
    for (int64_t i = 0; i < rows; i++) {
      for (int64_t j = 0; j < cols; j++) {
        res_data[j * rows + i] = x_data[i * cols + j];
      }
    }
  } else {
    /* For higher dimensions, just copy data (simplified) */
    memcpy(res->data, x->data, x->total_elements * sizeof(float));
  }
