...
struct AutocalContext {
    bool verbose;
};

AutocalContext *autocal_create(void)
{
    AutocalContext *ctx = (AutocalContext *) malloc(sizeof(AutocalContext));
    if (ctx)
        ctx->verbose = true;
    return ctx;
}
...
double autocal_measure_time(AutocalWorkload workload)
{
    // Warmup
    if (workload.setup)
        workload.setup(workload.context);
    for (int i = 0; i < 2; i++) {
        workload.execute(workload.context);
    }

    double start = nova_now_ms();
    for (int i = 0; i < workload.iterations; i++) {
        workload.execute(workload.context);
    }
    double end = nova_now_ms();

    if (workload.teardown)
        workload.teardown(workload.context);

    return (end - start) / workload.iterations;
}
...
void nova_autocal_run(void)
{
    ...
    printf("   CPU Tile: %dx%dx%d | Workers: %d%s\n", cpu_cal->matmul_tile_m,
           cpu_cal->matmul_tile_k, cpu_cal->matmul_tile_n, cpu_cal->worker_threads,
