...
static inline bool lookup_keyword(const char *str, uint32_t len,
                                  TokenType *out_type) {
...
static inline const char *skip_whitespace_simd(const char *ptr,
                                               const char *end) {
  ...
  while (ptr + 32 <= end) {
    ...
    __m256i is_ws =
        _mm256_or_si256(_mm256_or_si256(_mm256_cmpeq_epi8(chunk, space),
  ...
  while (ptr + 16 <= end) {
    ...
    uint8x16_t is_ws =
        vorrq_u8(vorrq_u8(vceqq_u8(chunk, space), vceqq_u8(chunk, tab)),
                 vorrq_u8(vceqq_u8(chunk, newline), vceqq_u8(chunk, carriage)));
    uint64_t low = vgetq_lane_u64(vreinterpretq_u64_u8(is_ws), 0);
    uint64_t high = vgetq_lane_u64(vreinterpretq_u64_u8(is_ws), 1);
...
static Token make_token(LexerContext *ctx, const char *start, uint32_t len,
                        TokenType type) {
  Token tok = {.start = start,
               .length = len,
               .type = type,
               .line = ctx->line,
               .column = ctx->column};
...
static void lex_number(LexerContext *ctx) {
  ...
  if (ctx->ptr < ctx->end && *ctx->ptr == '.' && ctx->ptr + 1 < ctx->end &&
      IS_DIGIT(ctx->ptr[1])) {
    is_float = true;
    ctx->ptr++; // Skip '.'
  ...
  uint32_t len = ctx->ptr - start;
  Token tok =
      make_token(ctx, start, len, is_float ? TOKEN_FLOAT : TOKEN_INTEGER);
  token_array_push(ctx->tokens, tok);
}
...
TokenArray *tokenize(const char *source, size_t length) {
  LexerContext ctx = {.source = source,
                      .ptr = source,
                      .end = source + length,
                      .line = 1,
                      .column = 1,
...
int main() {
  ...
  // Test program
  const char *test_code = "fn fibonacci(n: i64) -> i64 {\n"
                          "    if n <= 1 {\n"
                          "        yield n;\n"
                          "    }\n"
                          "    let a = fibonacci(n - 1);\n"
  ...
  for (uint32_t i = 0; i < tokens->count && i < 20; i++) {
    ...
    printf("  [%u:%u] %-10s ", tok->line, tok->column,
