...
NovaExecGraph *nova_exec_graph_create(NovaContext *ctx) {
  NovaExecGraph *graph = calloc(1, sizeof(NovaExecGraph));
  graph->ctx = ctx;
  graph->node_count = 0;
  graph->node_capacity = 64;
  graph->nodes = malloc(graph->node_capacity * sizeof(NovaExecGraphNode *));
...
void nova_exec_graph_destroy(NovaExecGraph *graph) {
  ...
  for (int i = 0; i < graph->node_count; i++) {
    NovaExecGraphNode *node = graph->nodes[i];
    if (node->inputs)
      free(node->inputs);
    if (node->outputs)
      free(node->outputs);
    ...
    if (node->type == NOVA_EXEC_NODE_OP && node->tensor) {
...
NovaExecGraphNode *nova_exec_graph_add_input(NovaExecGraph *graph,
                                             NovaTensor *t, const char *name) {
  if (graph->node_count >= graph->node_capacity) {
    graph->node_capacity *= 2;
    graph->nodes = realloc(graph->nodes,
                           graph->node_capacity * sizeof(NovaExecGraphNode *));
  }

  NovaExecGraphNode *node = calloc(1, sizeof(NovaExecGraphNode));
  node->id = graph->node_count;
  node->type = NOVA_EXEC_NODE_INPUT;
  node->tensor = t;
...
NovaExecGraphNode *nova_exec_graph_add_op(NovaExecGraph *graph,
                                          NovaExecGraphOpType op,
                                          NovaExecGraphNode **inputs,
  ...
  NovaExecGraphNode *node = calloc(1, sizeof(NovaExecGraphNode));
  node->id = graph->node_count;
  node->type = NOVA_EXEC_NODE_OP;
  node->op_type = op;
  node->input_count = input_count;
  node->inputs = malloc(input_count * sizeof(NovaExecGraphNode *));
  memcpy(node->inputs, inputs, input_count * sizeof(NovaExecGraphNode *));
  node->name = strdup(name);
  node->tensor = NULL;
...
static int64_t g_node_output_count(void *g, int64_t id) {
  ...
  int count = 0;
  for (int i = 0; i < graph->node_count; i++) {
    NovaExecGraphNode *n = graph->nodes[i];
    if (n->type != NOVA_EXEC_NODE_OP)
      continue;
...
static int64_t g_node_create(void *g, int32_t op, const int64_t *inputs,
  ...
  NovaExecGraphNode **input_ptrs =
      malloc(n_inputs * sizeof(NovaExecGraphNode *));
  ...
  int real_input_count = (op == NOVA_EXEC_OP_FUSED) ? n_inputs - 1 : n_inputs;
  int64_t fused_op_id = (op == NOVA_EXEC_OP_FUSED) ? inputs[n_inputs - 1] : 0;
  ...
  NovaExecGraphNode *new_node =
      nova_exec_graph_add_op(graph, (NovaExecGraphOpType)op, input_ptrs,
                             real_input_count, "FusedNode");
  if (op == NOVA_EXEC_OP_FUSED) {
...
static void g_node_replace_uses(void *g, int64_t old_id, int64_t new_id) {
  NovaExecGraph *graph = (NovaExecGraph *)g;
  NovaExecGraphNode *new_node = NULL;
...
static void g_node_delete(void *g, int64_t id) {
  ...
  if (id < graph->node_count) {
    ...
    // introduce NOVA_EXEC_NODE_DELETED type or just skip in executor loops.
    ...
    graph->nodes[id]->type = NOVA_EXEC_NODE_PLACEHOLDER;
  }
}
...
static NovaExecGraphVTable g_vt = {.node_count = g_node_count,
...
int nova_exec_graph_execute(NovaExecGraph *graph) {
  for (int i = 0; i < graph->node_count; i++) {
    ...
    if (node->type == NOVA_EXEC_NODE_PLACEHOLDER)
      continue;

    if (node->type == NOVA_EXEC_NODE_OP && !node->is_executed) {

      switch (node->op_type) {
      case NOVA_EXEC_OP_ADD:
        if (node->input_count == 2) {
          node->tensor =
              nova_tensor_add(node->inputs[0]->tensor, node->inputs[1]->tensor);
        }
        break;
      case NOVA_EXEC_OP_MATMUL:
        if (node->input_count == 2) {
          node->tensor = nova_tensor_matmul(node->inputs[0]->tensor,
                                            node->inputs[1]->tensor);
        }
        break;
      case NOVA_EXEC_OP_MUL:
        if (node->input_count == 2) {
          node->tensor =
              nova_tensor_mul(node->inputs[0]->tensor, node->inputs[1]->tensor);
        }
        break;
      case NOVA_EXEC_OP_ACTIVATION:
        // Support ReLU for now
        node->tensor = nova_tensor_relu(node->inputs[0]->tensor);
        break;
      case NOVA_EXEC_OP_FUSED: {
...
NovaTensor *nova_exec_graph_node_get_tensor(NovaExecGraphNode *node) {
