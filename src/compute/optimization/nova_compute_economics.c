...
NovaComputeEconomics *nova_economics_init(double budget)
{
    ...
    e->prices = (ResourcePrices) {.cpu_price_per_ms = 0.0001,
                                  ...
                                  .army_price_per_op = 0.00005}; // Ultra cheap
...
{
    return (cpu * e->prices.cpu_price_per_ms) + (gpu * e->prices.gpu_price_per_ms);
}
...
void nova_economics_track_usage(NovaComputeEconomics *e, uint64_t cpu, uint64_t gpu, size_t mem)
{
    ...
    e->current_session.memory_used_mb =
        mem > e->current_session.memory_used_mb ? mem : e->current_session.memory_used_mb;
