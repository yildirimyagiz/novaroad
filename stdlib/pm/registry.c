...
static char *g_registry_url = NULL;
static char *g_cache_dir = NULL;
static char *g_install_dir = NULL;
...
bool pm_download(NovaPackage *pkg, const char *dest_path) {
  if (!pkg || !pkg->tarball_url)
    return false;
  return http_download(pkg->tarball_url, dest_path);
}
...
bool pm_install(const char *name, const char *version) {
  ...
  if (version) {
    ...
    pkg = info->latest;
    info->latest = NULL; // Transfer ownership
