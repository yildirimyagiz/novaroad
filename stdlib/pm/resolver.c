/**
 * Novae Manager - Dependency Resolver Implementation
 */
...
bool version_satisfies(const char *version, VersionConstraint *c) {
  ...
  case VERSION_GTE:
    return version_compare(version, "") >= 0; // Simplified
...
static DepNode *create_node(const char *name, const char *version) {
  ...
  node->version = version ? strdup(version) : NULL;
  return node;
}
    ...
    if (sub && sub->success) {
      ...
      for (size_t j = 0; j < sub->count; j++) {
        ...
        if (!found) {
          ...
          sub->deps[j].name = NULL; // Transfer ownership
        }
      }
    }
